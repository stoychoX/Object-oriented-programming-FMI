...
Можем да реализираме колекция (контейнер) от обекти от различни типове (но с общ базов клас). ыказателите трябва да са от типа на базовия клас. При примера с фигурите (Shapes) видяхме следния пример:
```cpp
...
### Нетипови прараметри
Можем да пуснем определен тип в списъка на `templete`
```cpp
...
```
Сега, компилирайки `src.cpp`, ще имаме и дефинициите и декларациите на шаблонните функции във файла (след като се изпълни препроцесора). Слагодарение на това ще намерим нужните символи и няма да има нужда да ги пишем експлицитно. И двата метода се използват в зависимост от ситуацията (но `.hpp` файловете са предпочитани в повечето случаи).
