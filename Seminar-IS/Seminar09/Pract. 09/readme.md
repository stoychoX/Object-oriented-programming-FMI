...
Публичен интерфейс на класа:
 ...
 - `insert(element, index=0)` или `(insert(element&&, index=0))` - добавя число на позиция `index`. Ако `index >= size`, то да се добави в края. Ако не е указано кой е `index-a`, то той е 0
