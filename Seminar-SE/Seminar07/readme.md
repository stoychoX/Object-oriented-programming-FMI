...
За да имаме бързи конкатенации ще имплементираме класа с капацитет. Ако имаме достатъчно място в низа при конкатенация няма да заделяме нова памет а ще записваме в свободното място.

### Задача втора
Да се създаде клас `Censore`. Класът получава функция от по-висок ред със следната декларациа `bool (*)(char)`. Ако функцията върне истина то символа трябва да бъде цензуриран. 

Предефинирайте ```operator<<``` така, че да приема низ и да попълва потока със същия низ, но с цензурирани символи.
